...
plt.rcParams.update({
    ...
    "axes.labelsize": 22,     # x‑/y‑label font
...
def split_model_name(name: str) -> pd.Series:
    ...
    return pd.Series(
        [size, shape, epochs, lr, optimizer],
        index=["Size", "Shape", "Epochs", "Learning Rate", "Optimizer"],
    )
...
norm = mpl.colors.Normalize(vmin=results["MSE"].min(),
                            vmax=results["MSE"].max())
...
for (row, col), cell in tbl.get_celld().items():
    if row == 0:                                      # ── header
        cell.set_text_props(weight="bold", fontsize=18)   # larger header
        cell.set_facecolor("#dce6f2")
    else:                                             # ── data rows
        cell.set_fontsize(20)                             # body text
        if row % 2 == 0:
